...
public class SUPERCharacterAIO : MonoBehaviour{
    ...
    void Update(){
        if(!controllerPaused && !movementPaused){
        #region Input
        #if ENABLE_INPUT_SYSTEM
        ...
        /*
        RaycastHit p;
        if(Physics.SphereCast(playerCamera.transform.position,0.25f,playerCamera.transform.forward,out p,interactRange,interactableLayer,QueryTriggerInteraction.Ignore)){
            IInteractable i = p.collider.GetComponent<IInteractable>();
            if(i!=null){
        ...
        */
        ...
        RaycastHit hit;
        if (Physics.Raycast(ray, out hit, interactRange) && Time.timeScale != 0) {
            IInteractable i = hit.collider.GetComponent<IInteractable>();
            cakeslice.Outline j = hit.collider.GetComponent<cakeslice.Outline>();
            if(i!=null && j==null) {
                raycastHit = true;
                interactText.gameObject.SetActive(true);
            } else if (i!=null && j!=null && !j.eraseRenderer)
            {
                raycastHit = true;
                interactText.gameObject.SetActive(true);
            }
        ...
        // CAPN
        } else if (!controllerPaused && movementPaused){
                MouseXY.x = Input.GetAxis("Mouse Y");
                MouseXY.y = Input.GetAxis("Mouse X");
                mouseScrollWheel = Input.GetAxis("Mouse ScrollWheel");
                perspecTog = Input.GetKeyDown(perspectiveSwitchingKey_L);
...
public class SuperFPEditor : Editor{
    ...
    public override void OnInspectorGUI(){
        ...
        // CAPN
        t.interactText = (TextMeshProUGUI)EditorGUILayout.ObjectField(new GUIContent("Interact Text", "The text used to indicate interaction"), t.interactText, typeof(TextMeshProUGUI), true);
        // CAPN
